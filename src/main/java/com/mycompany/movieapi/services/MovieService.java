...
public class MovieService {
    ...
    public List<Movie> getAllMoviesByAccount(int customerID,int accountID) {
        return customerlist.get(customerID-1).getCustomerAccounts().get(accountID).getAccountMovies();
    }
    
    public Movie getMovieByID(int customerID,int accountID, int MovieID) {
        return customerlist.get(customerID-1).getCustomerAccounts().get(accountID).getAccountMovies().get(MovieID);
    }
	...
	return m;
     ...
     public void deleteeMovie(int CustomerID,int AccountID ,int MovieID){
         customerlist.get(CustomerID).getCustomerAccounts().get(AccountID).getAccountMovies().remove(MovieID);
